...
public class ModIcebergFeature extends Feature<BlockStateConfiguration> {
	public ModIcebergFeature(Codec<BlockStateConfiguration> pCodec) {
		super(pCodec);
	}
	...
	private void setIcebergBlock(BlockPos pPos, LevelAccessor pLevel, RandomSource pRandom, int pHeightRemaining, int pHeight, boolean pElliptical, boolean pPlaceSnow, BlockState pState) {
		...
		if (blockstate.getMaterial() == Material.AIR || blockstate.is(Blocks.SNOW_BLOCK) || blockstate.is(Blocks.ICE) || blockstate.is(Blocks.WATER)) {
			...
			if (pPlaceSnow && !blockstate.is(Blocks.WATER) && pHeightRemaining <= pRandom.nextInt(Math.max(1, pHeight / i)) + pHeight * 0.6D && flag) {
	...
	private double signedDistanceCircle(int pX, int pZ, BlockPos pCenter, int pRadius, RandomSource pRandom) {
		float f = 10.0F * Mth.clamp(pRandom.nextFloat(), 0.2F, 0.8F) / pRadius;
		return f + Math.pow(pX - pCenter.getX(), 2.0D) + Math.pow(pZ - pCenter.getZ(), 2.0D) - Math.pow(pRadius, 2.0D);
	}

	private double signedDistanceEllipse(int pX, int pZ, BlockPos pCenter, int pMajorRadius, int pMinorRadius, double pAngle) {
		return Math.pow(((pX - pCenter.getX()) * Math.cos(pAngle) - (pZ - pCenter.getZ()) * Math.sin(pAngle)) / pMajorRadius, 2.0D) + Math.pow(((pX - pCenter.getX()) * Math.sin(pAngle) + (pZ - pCenter.getZ()) * Math.cos(pAngle)) / pMinorRadius, 2.0D) - 1.0D;
	}
	...
	private int heightDependentRadiusRound(RandomSource pRandom, int pY, int pHeight, int pMajorAxis) {
		...
		float f1 = (1.0F - (float)Math.pow(pY, 2.0D) / (pHeight * f)) * pMajorAxis;
		if (pHeight > 15 + pRandom.nextInt(5)) {
			int i = pY < 3 + pRandom.nextInt(6) ? pY / 2 : pY;
			f1 = (1.0F - i / (pHeight * f * 0.4F)) * pMajorAxis;
		}
	...
	private int heightDependentRadiusEllipse(int pY, int pHeight, int pMaxRadius) {
		float f1 = (1.0F - (float)Math.pow(pY, 2.0D) / (pHeight * 1.0F)) * pMaxRadius;
		return Mth.ceil(f1 / 2.0F);
	}
	...
	private int heightDependentRadiusSteep(RandomSource pRandom, int pY, int pHeight, int pMaxRadius) {
		...
		float f1 = (1.0F - pY / (pHeight * f)) * pMaxRadius;
		return Mth.ceil(f1 / 2.0F);
	}

	private static boolean isIcebergState(BlockState pState) {
		return pState.is(Blocks.PACKED_ICE) || pState.is(Blocks.SNOW_BLOCK) || pState.is(Blocks.BLUE_ICE) || pState.is(BlockRegistry.SMOOTH_ICE.get());
	}
